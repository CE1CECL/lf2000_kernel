 ...
 *
 * based on the S3C-hsotg.c driver
...
/* FIXME: sesters, just for debugging */
#define DEBUG 1

extern unsigned int detect_usb_charger(void);
				...
				struct s3c_hsotg_req *hs_req)
{
	...
	/* If the cable is believed to be disconnected and the core will not accept
     * any more data, bail early so that the irq doesn't get re-enabled and continue
     * to fire.
     */
	if(!hsotg->conn_state && can_write == 0)
	{
		dev_dbg(hsotg->dev, "%s: Cable is probably not connected, bailing out\n",
			__func__);
		return -ENOSPC;
	}
				...
				bool continuing)
{
	...
	if (index != 0) {
		if (hs_ep->req && !continuing) {
			dev_err(hsotg->dev, "%s: active request; ep%d\n", 
				__func__, index);
			WARN_ON(1);
			return;
...
#if 1	/* 15mar12  Copied from u-boot gadget driver*/
			ep->halted = (set ? 1 : 0);
#endif
...
#if 1	/* 15mar12 Copied this change from the u-boot driver.  It's needed
	 *	   in order to pass Chapter 9 compliance tests.
	 *	   After a stall, we must queue a request for next packet,
...
static void s3c_hsotg_update_connected_state(unsigned long data)
{
	...
	{
		dev_info(hsotg->dev, "Actually a wall charger, false alarm... \n");
	}
}
...
irq_retry:
	...
	if (gintsts & S3C_GINTSTS_USBRst) {
		...
		/* Set a debounce timer for connection state */
#if 1	/* 19mar12  Reenable to generate VBUS message (TTPro #2915) */
//#if 0	/* 16mar12  After disabling next statement, driver passes Chap 9 test */
			       ...
			       const struct usb_endpoint_descriptor *desc)
{
	...
	/* Clear stall bit when enabling endpoint */
	epctrl &= ~(S3C_DxEPCTL_Stall);
...
int s3c_hsotg_start(struct usb_gadget_driver *driver,
...
int s3c_hsotg_stop(struct usb_gadget_driver *driver)
{
	struct s3c_hsotg *hsotg = our_hsotg;
	int ep;
...
static int state_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "GOTGCTL  =0x%08x, GOTGINT  =0x%08x, GAHBCFG       =0x%08x, GUSBCFG       =0x%08x\n",
		 readl(regs + S3C_GOTGCTL),
		 readl(regs + S3C_GOTGINT),
		 readl(regs + S3C_GAHBCFG),
		 readl(regs + S3C_GUSBCFG));

	seq_printf(seq, "GRSTCTL  =0x%08x, GINTSTS  =0x%08x, GINTMSK       =0x%08x, GRXSTSR       =0x%08x\n",
		 readl(regs + S3C_GRSTCTL),
		 readl(regs + S3C_GINTSTS),
		 readl(regs + S3C_GINTMSK),
		 readl(regs + S3C_GRXSTSR));

	seq_printf(seq, "GRXSTSP  =0x%08x, GRXFSIZ  =0x%08x, GNPTXFSIZ     =0x%08x, GNPTXSTS      =0x%08x\n",
		 readl(regs + S3C_GRXSTSP),
		 readl(regs + S3C_GRXFSIZ),
		 readl(regs + S3C_GNPTXFSIZ),
		 readl(regs + S3C_GNPTXSTS));

	seq_printf(seq, "GI2CCTL  =0x%08x, GPVNDCTL =0x%08x, GGPIO         =0x%08x, GUID          =0x%08x\n",
		 readl(regs + S3C_GI2CCTL),
		 readl(regs + S3C_GPVNDCTL),
		 readl(regs + S3C_GGPIO),
		 readl(regs + S3C_GUID));

	seq_printf(seq, "GSNPSID  =0x%08x, GHWCFG1  =0x%08x, GHWCFG2       =0x%08x, GHWCFG3       =0x%08x\n",
		 readl(regs + S3C_GSNPSID),
		 readl(regs + S3C_GHWCFG1),
		 readl(regs + S3C_GHWCFG2),
		 readl(regs + S3C_GHWCFG3));

	seq_printf(seq, "GHWCFG4  =0x%08x, GLPMCFG  =0x%08x, PHYPOR        =0x%08x, TESTPARM3     =0x%08x\n",
		 readl(regs + S3C_GHWCFG4),
		 readl(regs + S3C_GLPMCFG),
		 readl(regs + S3C_PHYPOR),
		 readl(regs + S3C_TESTPARM3));

	seq_printf(seq, "TESTPARM4=0x%08x, LINCTRL  =0x%08x, TESTPARM6     =0x%08x, TESTPARM7     =0x%08x\n",
		 readl(regs + S3C_TESTPARM4),
		 readl(regs + S3C_LINCTRL),
		 readl(regs + S3C_TESTPARM6),
		 readl(regs + S3C_TESTPARM7));

	seq_printf(seq, "TESTPARM8=0x%08x, TESTPARM9=0x%08x, USB_OTG_CLKENB=0x%08x, USB_OTG_CLKGEN=0x%08x\n",
...
static int __devexit s3c_hsotg_remove(struct platform_device *pdev)
{
	...
	del_timer( &(hsotg->conn_state_timer) );
